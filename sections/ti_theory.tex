...
\end{align}
It's not Gauge invariant, which makes it difficult to be useful for different measurements. However, if you begin \textbf{AND} end in the same configuration point (ie, a closed loop) then this value is Gauge invariant! This imposes a few restrictions on the system to achieve a result.
\begin{itemize}
...
\end{itemize}
In 3D systems, the Berry phase over some path $\Gamma$ enclosing a surface $S$, then Stokes theorem lets you specify the \textbf{Berry curvature} 
\begin{align}
...
\textcolor{red}{Todo: Fill out some basic theory and why the Berry phase solves this problem} %TODO: Aharonov–Bohm effect
